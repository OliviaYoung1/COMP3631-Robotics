...
class GoToPose(Node):
    def __init__(self):
        ...
        #create a client for the action, sending NavigateToPose goals to the nvigate_to_pose actions
    ...
    def send_goal(self, x, y, yaw):
        ...
        #Set the desired goal, pos and orientation for the action message were sending
        # Position
        goal_msg.pose.pose.position.x = x
        goal_msg.pose.pose.position.y = y

        # Orientation- convert an Euler angle to a quaternion representation
        goal_msg.pose.pose.orientation.z = sin(yaw / 2)
        goal_msg.pose.pose.orientation.w = cos(yaw / 2)

        self.action_client.wait_for_server()
        #send the goal here v
        self.send_goal_future = self.action_client.send_goal_async(goal_msg, feedback_callback=self.feedback_callback)
        self.send_goal_future.add_done_callback(self.goal_response_callback)

    #aynchronous method providing useful info about the progress of the action
...
def main(args=None):
    ...
    #creates a GoToPose node (a class weve defined) and then uses send_goal method
    go_to_pose = GoToPose()
    go_to_pose.send_goal(4.98, -3.55, -0.00143)  # example coordinates
